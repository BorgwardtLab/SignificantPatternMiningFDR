...
int qsort_pair_cmp(const void* x, const void* y){
  if ( ((PV_ID_PAIR*)x)->pv < ((PV_ID_PAIR*)y)->pv ) return (-1);
  else return ( ((PV_ID_PAIR*)x)->pv > ((PV_ID_PAIR*)y)->pv );
}
void qsort_pair( PV_ID_PAIR* a, int siz ){
    qsort ( a, siz, sizeof(PV_ID_PAIR), qsort_pair_cmp );
}
