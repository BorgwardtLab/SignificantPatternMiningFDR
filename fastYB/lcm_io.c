...
void LCM_output_itemset2(){
  ...
  FASTO_cpy2(LCM_testable_patterns, LCM_itemsetbuf, LCM_p);
  double pv = fisher_pval_pre(LCM_ap, LCM_frq);
  if(pv < delta){
    printf("%s: %e\n", LCM_testable_patterns, pv);
  }
  //LCM_output_itemset();
  //LCM_i_testable++;
  //printf("%s (%d, %d) \n", &LCM_testable_patterns[MAX_ITEMSET_STR_LEN*LCM_i_testable], LCM_frq, LCM_ap);
}
...
void LCM_solution(){
  ...
  /* END OF MODICATIONS */
  if(LCM_print_flag&1) LCM_output_itemset2(); //PAOLO
}
...
void LCM_print_last(int item, int frq){
  ...
  LCM_frq = frq;
  LCM_additem(item);
  process_solution0_ap_perm(LCM_frq); //PAOLO
  LCMFREQ_solution();
