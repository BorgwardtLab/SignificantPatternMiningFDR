...
void LCM_output_itemset2(){
  if(LCM_print_flag)FASTO_cpy2(&LCM_testable_patterns[MAX_ITEMSET_STR_LEN*LCM_i_testable], LCM_itemsetbuf, LCM_p);
  LCM_testable_occurrences[LCM_i_testable].pv = fisher_pval_pre(LCM_ap, LCM_frq); //((long long)LCM_frq)*1000000000 + LCM_ap;
  LCM_testable_occurrences[LCM_i_testable].id = LCM_i_testable;
  //printf("%s (%d, %d, %e) \n", &LCM_testable_patterns[MAX_ITEMSET_STR_LEN*LCM_i_testable], LCM_frq, LCM_ap, LCM_testable_occurrences[LCM_i_testable].pv); fflush(stdout);
  LCM_i_testable++;
  //LCM_output_itemset();
}
...
#endif
  LCM_sc[LCM_itemsett]++;
  LCM_patterns_with_supp[LCM_frq]++;  // PAOLO
  if(LCM_frq < LCM_th){
    printf("NOPE solution %d\n", LCM_frq);
    fflush(stdout);
  }
  /* MODIFICATIONS TO KEEP TRACK OF CURRENT ITEMSET */
  //print_current_itemset();
  /* END OF MODICATIONS */
  if(LCM_sign) LCM_output_itemset2(); //PAOLO
}
...
void LCMFREQ_solution(){
	if(1 | LCM_print_flag&1) LCMFREQ_output_itemset(LCM_add.q+LCM_add.t);
//  int i, cc=1, s = LCM_itemset.t;
  int i, cc=1, s = LCM_itemsett;
  for(i=0; i<=LCM_add.t; i++){
    LCM_sc[s] += cc;
    LCM_patterns_with_supp[LCM_frq] += cc;  // PAOLO
    cc = cc*(LCM_add.t - i) / (i+1);
    s++;
  }
  if(LCM_frq < LCM_th){
    printf("NOPE frq solution %d\n", LCM_frq);
    fflush(stdout);
  }
}
...
void LCM_print_last(int item, int frq){
  ...
  LCM_frq = frq;
  LCM_additem(item);
  if(LCM_sign) process_solution0_ap(LCM_frq);
  LCMFREQ_solution();
...
void LCM_output(){
  ...
  printf("%d\n", n);
  for(i=0; i<=ii; i++) printf("%d: %d\n", i, LCM_sc[i] );
  int cntpt = 0;
  for ( i=0 ; i<=LCM_trsact_num ; i++ ) cntpt += LCM_patterns_with_supp[i];
  printf ("Cnt pt %d\n", cntpt);
