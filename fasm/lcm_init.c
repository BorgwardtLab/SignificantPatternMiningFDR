...
int LCM_init(char* trans_file){
  ...
  /* END OF MODIFICATIONS */
  n = LCM_BM_load(trans_file);   /* loding file of bitmap version */
  ...
  /* MODIFICATIONS FOR KEEPING TRACK OF TRANSACTIONS */
  transaction_keeping_init(trans_file);
  /* END OF MODIFICATIONS */
...
/***********************************************************************/
  ...
  if(LCM_print_flag&1){
	/* MODIFICATIONS FOR FAST WY ALGORITHIM */
    FASTO_init_perm("out.txt", LCM_Eend);//Check argv[3] changed to argv[2] since LCM_th is no longer an input argument
    /* END OF MODIFICATIONS */
