 ...
 * */
void fasm_init(double sig_th, int testable_size, char *labels_file){
	LCM_testable_size = testable_size+1;
	LCM_i_testable = 0;
	if(LCM_print_flag){
		malloc2(LCM_testable_patterns, char, LCM_testable_size*MAX_ITEMSET_STR_LEN, "LCM_testable_patterns");
		memset(LCM_testable_patterns, 0, LCM_testable_size*MAX_ITEMSET_STR_LEN);
	}
	malloc2(LCM_testable_occurrences, PV_ID_PAIR, LCM_testable_size, "LCM_testable_occurrences");
...
void fasm_end(){
	...
	free(labels);
	free(LCM_testable_occurrences);
	if(LCM_print_flag){
		free(LCM_testable_patterns);
	}
}
